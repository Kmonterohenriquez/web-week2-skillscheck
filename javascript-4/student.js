...
// CODE HERE
const faveColors=['#9CECFB', '#65C7F7','#0052D4'];
...
const me={
    firstName: 'Kevin',
    superHeroName: 'Kakaroto',
    homeTown: 'Santo Domingo',
    superPowers: ['skateboarding', 'crime fighting', 'crime inciting'],
...
function setColor(arr){
    arr.splice(3);
    const newArray =arr.map((curr)=> curr === 'blue'? curr ='#4D4DFF': curr );
    background(...newArray);
}
...
function setPowers(arr){
    ...
    return arr.forEach((curr)=> createLi(curr));
}
...
function redactInfo(obj){
    Object.keys(obj).forEach((curr)=> obj[curr] ='redacted');
    redacted()
}